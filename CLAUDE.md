...
- `MultiUse = true` is needed so `screen.UpgradedRarity` doesn't block the whole screen after 1 upgrade
- `UpgradeMouseOverUpgradeChoice` fires every frame while hovering, so we wrap it and track `screen._rarifyLastButton`: skip if `screen.MouseOverButton == screen._rarifyLastButton`. This gives 1 upgrade per hover-enter event; holding the mouse still does nothing. To upgrade a boon multiple tiers, hover away and back each time.
